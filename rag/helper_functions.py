...
def get_query(id=None):
    idquery = f"AND trim(lower(t.token_address)) = trim(lower('{id}'))" if id else ""
    return text(f"""
            WITH dexs_group AS (
            ...
            SELECT t.id, lower(trim(t.token_address)) as token_address, t.anti_whale_modifiable, t.buy_tax, t.can_take_back_ownership, t.cannot_buy, 
